...
Cool.updateVolume = function()
	awful.spawn.easy_async("wpctl get-volume @DEFAULT_AUDIO_SINK@", function(stdout, _, _, _)
		...
		if string.find(stdout, "MUTED") then
			...
			widgets.volume_text:set_markup_silently(
				'<span strikethrough="true" strikethrough_color="'
					.. beautiful.catpuccin.red
					.. '">'
					.. Cool.volume
					.. "%"
					.. "</span>"
...
Cool.updateBattery = function()
	awful.spawn.easy_async_with_shell(
		...
		function(stdout, _, _, _)
			...
			if
				(tonumber(string.sub(stdout, -5, -3)) <= batteryWarningCritical)
				and (string.find(stdout, "discharging"))
			then
				BatWarning = naughty.notify({
					title = "BATTERY IS JUST " .. widgets.batteryText.text,
					text = "BHAI JALDI CHARGING PE LAGA",
					preset = naughty.config.presets.critical,
					timeout = 0,
					replaces_id = BatWarning.id,
				})
				widgets.batteryInnerBar.bg = create.twoSolidColors(
					0,
					22,
					0,
					0,
					(tonumber(string.sub(stdout, -5, -3)) / 100),
					beautiful.catpuccin.red,
			...
			elseif
				(tonumber(string.sub(stdout, -5, -3)) <= batteryWarning) and (string.find(stdout, "discharging"))
			then
				BatWarning = naughty.notify({
					title = "BATTERY IS JUST " .. widgets.batteryText.text,
					text = "charge kar bhai",
					timeout = 20,
					replaces_id = BatWarning.id,
				})
				widgets.batteryInnerBar.bg = create.twoSolidColors(
...
widgets.volumeBox.buttons = gears.table.join(
	...
	awful.button({}, 3, function()
		awful.spawn.with_shell("kcmshell6 kcm_pulseaudio")
	end) -- Right Click
)

Cool.change_bat = function(by)
	widgets.batteryText.text = tostring(tonumber(widgets.batteryText.text:gsub("%%", ""), 10) + by) .. "%"
end

widgets.batteryContainer.buttons = gears.table.join(
...
widgets.bluetoothBox.buttons = gears.table.join(
	...
	awful.button({}, 3, function()
		awful.spawn.with_shell("kcmshell6 kcm_bluetooth")
	end) -- Middle Click
)
...
Cool.setWallpaper = function(file, adjustment)
	if adjustment == "fill" then
		for s in screen do
			gears.wallpaper.maximized(file, s)
		end
	else
		for s in screen do
			gears.wallpaper.fit(file, s)
		end
	end
	Cool.wallpaperInfo()
end
...
Cool.setAsCurrentWallpaper = function()
	if wallOffset ~= 0 then
		...
		awful.spawn.with_shell(
			"~/.config/awesome/wallpaperSources/fileget/fileget --setFile '" .. Cool.active_wallp .. "'"
		)
	end
	Cool.wallpaperInfo()
end
...
Cool.wallpaperInfo = function()
	local file =
		Cool.active_wallp:gsub("/home/harshit/Harshit_Work/Funny_Stuff/", ""):gsub("/home/harshit/Harshit_Work/", "")
...
Cool.updateBluetooth = function()
	awful.spawn.easy_async_with_shell("bluetoothctl devices Connected | grep Device", function(stdout, _, _, _)
		if stdout ~= prev_bt_output then
			...
			for _, device_info in pairs(yo) do
				...
				elseif string.find(device.name, "Toad") then
					img = beautiful.icon.mouse
				else
